...
public struct AMIBenchmark {

public static func runAMISDMBenchmark(
    ...
    for meetingId in commonMeetings {
        ...
        do {
            ...
            let audioSamples = try await AudioProcessing.loadAudioFile(path: audioPath.path)
            ...
            let groundTruthSpeakerCount = AMIGroundTruthLoader.getGroundTruthSpeakerCount(
                for: meetingId)

            // Load ground truth annotations
            let groundTruth = await AMIGroundTruthLoader.loadAMIGroundTruth(
    ...
    BenchmarkResultsDisplay.printBenchmarkResults(
    ...
    if let outputFile = outputFile {
        ...
        do {
            try await OutputFormatter.saveBenchmarkResults(summary, to: outputFile)
...
public static func runAMIIHMBenchmark(
    manager: DiarizerManager, config: DiarizerConfig, outputFile: String?,
    autoDownload: Bool, singleFile: String? = nil, iterations: Int = 1
) async {
    let homeDir = FileManager.default.homeDirectoryForCurrentUser
