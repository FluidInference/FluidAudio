...
public static func runBenchmark(arguments: [String]) async {
...
// MARK: - Download Dataset Command

public static func downloadDataset(arguments: [String]) async {
    var dataset = "all"
...
// MARK: - Process File Command

public static func processFile(arguments: [String]) async {
    ...
    do {
        let audioSamples = try await AudioProcessing.loadAudioFile(path: audioFile)
        ...
        if let outputFile = outputFile {
            try await OutputFormatter.saveResults(output, to: outputFile)
            print("💾 Results saved to: \(outputFile)")
        } else {
            await OutputFormatter.printResults(output)
        }
