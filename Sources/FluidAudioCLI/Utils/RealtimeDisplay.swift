    ...
    actor RealtimeDisplay {
        ...
        private func renderStream(streamId: UUID, display: StreamDisplay) {
            ...
            for line in display.lines {
                ...
                print(
                    "\(ANSI.dim)[\(timeStr)]\(ANSI.reset) \(textColor)\(line.text)\(suffix)\(ANSI.reset)"
                )
            }
            ...
            if display.metrics.chunkCount > 0 {
                ...
                if let ttfw = display.metrics.timeToFirstWord {
                    metricsLine +=
                        "⏱️  TTFW: \(ANSI.green)\(String(format: "%.2f", ttfw))s\(ANSI.reset) "
                }
                ...
                if display.metrics.rtfx > 0 {
                    ...
                    metricsLine +=
                        "│ 📊 RTFx: \(rtfxColor)\(String(format: "%.1f", display.metrics.rtfx))x\(ANSI.reset) "
                }

                // Average latency
                if display.metrics.averageLatency > 0 {
                    metricsLine +=
                        "│ ⚡ Latency: \(ANSI.cyan)\(String(format: "%.3f", display.metrics.averageLatency))s\(ANSI.reset)"
                }
        ...
        func printSummary() {
            ...
            for (_, display) in streams.sorted(by: { $0.value.name < $1.value.name }) {
                ...
                print(
                    "  Total audio duration: \(String(format: "%.1f", display.metrics.totalAudioDuration))s"
                )
                print(
                    "  Total processing time: \(String(format: "%.2f", display.metrics.totalProcessingTime))s"
                )
                ...
                if display.metrics.rtfx > 0 {
                    let capability =
                        display.metrics.rtfx >= 1.0 ? "realtime capable" : "⚠️  below realtime"
                    print(
                        "  Real-time factor: \(String(format: "%.2f", display.metrics.rtfx))x \(capability)"
                    )
                }

                if display.metrics.averageLatency > 0 {
                    print(
                        "  Average latency: \(String(format: "%.3f", display.metrics.averageLatency))s"
                    )
                }
            }
        }
