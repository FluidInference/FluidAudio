...
public final class AsrManager {
    ...
    // The decoder is stateful, therefore we need to keep track of the state for each audio source
    private var microphoneDecoderState = DecoderState()
    private var systemAudioDecoderState = DecoderState()
    ...
    public func cleanup() {
        ...
        systemAudioDecoderState = DecoderState()
    ...
    public func transcribe(_ audioSamples: [Float], source: AudioSource) async throws -> ASRResult {
        ...
        case .system:
            return try await transcribeWithState(
                audioSamples, decoderState: &systemAudioDecoderState)
        }
    }
