...
/// Processes audio chunks from a streaming source with proper overlap handling.
/// Memory-efficient alternative to ChunkProcessor for large files.
struct StreamingChunkProcessor {
    private let logger = AppLogger(category: "StreamingChunkProcessor")
    ...
    private var maxModelSamples: Int { 240_000 }
    ...
    /// Process audio from a streaming sample source.
    /// - Parameters:
    ///   - sampleSource: Source providing samples on demand
    ///   - totalSamples: Total number of samples in the source
    ///   - manager: ASR manager for transcription
    ///   - startTime: Processing start time for metrics
    ///   - progressHandler: Optional progress callback
    /// - Returns: Transcription result
    func process(
        from sampleSource: StreamingAudioSampleSource,
        totalSamples: Int,
    ...
    private func readSamples(
        from source: StreamingAudioSampleSource,
        offset: Int,
        count: Int
    ) throws -> [Float] {
        var buffer = [Float](repeating: 0, count: count)
        try buffer.withUnsafeMutableBufferPointer { pointer in
            try source.copySamples(into: pointer.baseAddress!, offset: offset, count: count)
        }
        return buffer
    }
    ...
    // MARK: - Token Merging (same logic as ChunkProcessor)
