...
actor AudioBuffer {
    private let logger = Logger(subsystem: "com.fluidinfluence.asr", category: "AudioBuffer")
    ...
    func getChunk(size: Int) -> [Float]? {
        logger.debug("getChunk: size=\(size), available=\(self.count)")
        ...
        logger.debug("Advanced buffer: oldReadPos=\(oldReadPosition), newReadPos=\(self.readPosition), consumed=\(size), remaining=\(self.count)")
    ...
    func getChunkWithPartial(requestedSize: Int, allowPartial: Bool = false) -> [Float]? {
        logger.debug("getChunkWithPartial: requestedSize=\(requestedSize), available=\(self.count), allowPartial=\(allowPartial)")
