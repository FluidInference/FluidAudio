...
public struct StyleTTS2AneComputeUnits: Sendable, Equatable {
    ...
    public static let `default` = StyleTTS2AneComputeUnits()

    /// CPU+GPU only (skip ANE entirely). Useful for a baseline.
    public static let cpuAndGpu = StyleTTS2AneComputeUnits(
    ...
    public static let allAne = StyleTTS2AneComputeUnits(
    ...
    public static let cpuOnly = StyleTTS2AneComputeUnits(
    ...
    func units(for stage: StyleTTS2AneStage) -> MLComputeUnits {
        switch stage {
...
/// holds.
public actor StyleTTS2AneModelStore {

    private let logger = AppLogger(category: "StyleTTS2AneModelStore")
    private let directory: URL?
    private let computeUnits: StyleTTS2AneComputeUnits

    private var models: [StyleTTS2AneStage: MLModel] = [:]
    ...
    public func loadIfNeeded(
        ...
        let bundleRoot =
            try await StyleTTS2AneResourceDownloader
        ...
        for stage in StyleTTS2AneStage.allCases {
            ...
            guard FileManager.default.fileExists(atPath: url.path) else {
                throw StyleTTS2AneError.modelNotLoaded(stage.bundleName)
            }
    ...
    public func model(for stage: StyleTTS2AneStage) throws -> MLModel {
