...
enum KokoroChunker {
    ...
    private static func resolvePhonemes(
        ...
        if phonemes == nil {
            phonemes = EnglishG2PHelpers.stemInflected(normalized, lexicon: lexicon, allowed: allowed)
        }
        ...
        if phonemes == nil,
            let spelledTokens = EnglishG2PHelpers.spelledOutTokens(for: normalized),
            !spelledTokens.isEmpty
        {
            ...
            for spelled in spelledTokens {
                ...
                if segment == nil, let fallback = EnglishG2PHelpers.letterPronunciations[spelled] {
        ...
        if phonemes == nil, let fallback = EnglishG2PHelpers.letterPronunciations[normalized] {
            let filtered = fallback.filter { allowed.contains($0) }
            if !filtered.isEmpty {
                phonemes = filtered
            }
