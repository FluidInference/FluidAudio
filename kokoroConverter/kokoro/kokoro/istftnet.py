...
class AdaIN1d(nn.Module):
    ...
    def project_style(self, s: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        h = self.fc(s)
        h = h.view(h.size(0), h.size(1), 1)
        gamma, beta = torch.chunk(h, chunks=2, dim=1)
        return gamma, beta

    def forward(self, x, s=None, style_stats=None):
        if style_stats is None:
            if s is None:
                raise ValueError("Either style tensor or precomputed style_stats must be provided")
            gamma, beta = self.project_style(s)
        else:
            gamma, beta = style_stats
...
class AdaINResBlock1(nn.Module):
    ...
    def _snake_fp16(self, x: torch.Tensor, alpha: torch.Tensor) -> torch.Tensor:
        target_dtype = x.dtype
        sin_dtype = torch.float16 if target_dtype == torch.float32 else target_dtype
        sin_input = (alpha * x).to(sin_dtype)
        snake = torch.sin(sin_input)
        snake = snake * snake
        snake = snake.to(target_dtype)
        inv_alpha = torch.reciprocal(alpha.to(target_dtype))
        return x + inv_alpha * snake

    def prepare_style(self, s: torch.Tensor):
        stats1 = [n.project_style(s) for n in self.adain1]
        stats2 = [n.project_style(s) for n in self.adain2]
    ...
    def forward(self, x, s=None, style_stats=None):
        if style_stats is None:
            if s is None:
                raise ValueError("Either style tensor or precomputed style_stats must be provided")
            stats1, stats2 = self.prepare_style(s)
        else:
            stats1, stats2 = style_stats

        for c1, c2, n1, n2, a1, a2, stats1_pair, stats2_pair in zip(
            self.convs1,
            self.convs2,
            self.adain1,
            self.adain2,
            ...
            xt = n1(x, style_stats=stats1_pair)
            xt = self._snake_fp16(xt, a1)
            xt = c1(xt)
            xt = n2(xt, style_stats=stats2_pair)
            xt = self._snake_fp16(xt, a2)
            xt = c2(xt)
            x = xt + x
        return x

    def remove_weight_norm_(self) -> None:
        """Detach weight norm reparameterization for inference/export."""
