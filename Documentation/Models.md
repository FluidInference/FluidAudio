...
| **StyleTTS2 ANE (7-stage)** ([FluidAudio#583](https://github.com/FluidInference/FluidAudio/pull/583), [HF](https://huggingface.co/FluidInference/StyleTTS-2-coreml/tree/main/ANE)) | LibriTTS multi-speaker StyleTTS2 split into 7 CoreML stages mirroring Kokoro-ANE's shape (PLBert / PostBert / Alignment / DiffusionStep / Prosody / Noise / Vocoder). 6 of 7 stages pinned to the Neural Engine; Noise stays on `.all` because SineGen's phase accumulator needs fp32. ~330 MB after int8 palettization, ≤510 IPA chars per call, T_a ≤ 2000 acoustic frames. 17-voice named catalog (`--voice-name vinay`) plus arbitrary `ref_s.bin` blobs. The 5-step ADPM2 sampler runs in Swift (per-step UNet invoked 11× per utterance). | Re-cut from the legacy 4-graph StyleTTS2 by carrying over the Kokoro-ANE conversion patterns (BiLSTM unroll, fixed-shape Prosody, fp32 SineGen split, cos-Snake patch). See [StyleTTS2.md](TTS/StyleTTS2.md) and [mobius blocker rationale](https://github.com/FluidInference/mobius/blob/main/models/tts/styletts2/coreml/ANE.md). |
...
| StyleTTS2 ANE (7-stage) | [FluidInference/StyleTTS-2-coreml/tree/main/ANE](https://huggingface.co/FluidInference/StyleTTS-2-coreml/tree/main/ANE) (shared assets at repo root: `voices/`, `config.json`, `constants/text_cleaner_vocab.json`) |
