...
class KokoroCompleteCoreML(nn.Module):
    ...
    def __init__(self, model, bert, bert_encoder, predictor, device="cpu", samples_per_frame=600, max_frames=1200):
        super().__init__()
        self.model = model
        self.bert = bert
...
mlmodel.save("kokoro_24_30s_fp16.mlpackage")
...
# %%
ML Programs now let you override compute units at block granularity. If profiling shows
specific layers perform better on CPU (e.g. the small dense heads), you can tag them
before export:

with ct.utils.profile(mlmodel) as profiler:
  profiler.set_compute_unit("predictor/dense_0", ct.ComputeUnit.CPU_ONLY)

# %%
from coremltools.om import Model
model = Model(mlmodel)
model.summary()
...
SAMPLE_RATE = 24000
MAX_TOKENS = 508   # must match export

def pad_to_max(input_ids: torch.Tensor, max_tokens: int):
    original_len = min(input_ids.shape[1], max_tokens)

    if input_ids.shape[1] < max_tokens:
        repeats = (max_tokens + input_ids.shape[1] - 1) // input_ids.shape[1]
        input_ids = input_ids.repeat(1, repeats)[:, :max_tokens]
    else:
        input_ids = input_ids[:, :max_tokens]

    return input_ids, original_len
...
# %%
model = run_coreml_with_pt_inputs("kokoro_24_30s_fp16.mlpackage", "in-groups. in-groups. in-groups.", "af_heart")
...
run("I can't believe we finally made it to the summit after climbing for twelve exhausting hours through wind and rain, but wow, this view of the endless mountain ranges stretching to the horizon makes every single difficult step completely worth the journey.")
