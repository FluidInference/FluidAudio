...
- **Inverse Text Normalization (ITN)**: Post-process ASR output to convert spoken-form to written-form ("two hundred" → "200"). See [text-processing-rs](https://github.com/FluidInference/text-processing-rs). Optional: ASR-only apps can drop the ~18 MB engine with `traits: []` (Swift 6.1+), see [PostProcessing.md](Documentation/ASR/PostProcessing.md#opting-out-of-the-engine)
